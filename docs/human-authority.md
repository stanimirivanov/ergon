...
> a resolver for one tenant. This is evidence, not authentication or approval.
...
caller is the actor or a trusted identity provider. Authentication, provider
trust configuration, revocation, approval decisions, and authorization remain
separate. Opaque subjects and evidence are retained as audit history for the
tenant lifetime; tenant deletion and a broader privacy-retention policy are not
